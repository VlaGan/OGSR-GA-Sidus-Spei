...
void CActor::IR_OnKeyboardPress(int cmd)
{
    ...
    case kCAM_1: {
        if (Core.Features.test(xrCore::Feature::actor_legs))
        {
            ...
            CActor* pActor = static_cast<CActor*>(this);
            if (!pActor->IsFirstEye()){
                CCustomOutfit* pOutf = smart_cast<CCustomOutfit*>(pActor->inventory().ItemFromSlot(OUTFIT_SLOT));
                if (pOutf)
                {
                    shared_str outf = pOutf->GetCurrOutfLeg();
                    if (outf != NULL)
                    {
                        ChangeVisual(outf);
                        /*}
                        ...
                        {
                            shared_str Outf = pActor->GetDefaultVisualOutfit_legs();
                            ChangeVisual(Outf);*/
                    }
                }
                else
    ...
    case kCAM_2: {
        if (Core.Features.test(xrCore::Feature::actor_legs))
        {
            ...
            if (pOutf)
            {
                shared_str outf = pOutf->GetCurrOutf();
                if (outf != NULL)
                {
                    ChangeVisual(outf);
                }
    ...
    case kCAM_3: {
        ...
        {
            CActor* pActor = static_cast<CActor*>(this);
            shared_str Outf = pActor->GetDefaultVisualOutfit();
            ChangeVisual(Outf);
            cam_Set(eacFreeLook);
        }*/
        if (Core.Features.test(xrCore::Feature::actor_legs))
