...
void CUICellItem::ColorizeItems(std::initializer_list<CUIDragDropListEx*> args)
{
    ...
    auto WpnScanner = [&ColorizeSects](CWeaponMagazined* Wpn) {
        ...
        if (Wpn->BayonetAttachable())
            ColorizeSects.push_back(Wpn->GetBayonetName());
        if (Wpn->RailAttachable())
            ColorizeSects.push_back(Wpn->GetRailName());
        if (Wpn->LaserAttachable())
            ColorizeSects.push_back(Wpn->GetLaserName());
        if (Wpn->TorchAttachable())
            ColorizeSects.push_back(Wpn->GetTorchName());
        if (Wpn->TactHandlerAttachable())
