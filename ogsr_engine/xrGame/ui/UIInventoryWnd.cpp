...
void CUIInventoryWnd::Init()
{
    ...
    m_pUISlotBagList = xr_new<CUIDragDropListEx>();
    AttachChild(m_pUISlotBagList);
    m_pUISlotBagList->SetAutoDelete(true);
    xml_init.InitDragDropListEx(uiXml, "dragdrop_slot_bag", 0, m_pUISlotBagList);
    BindDragDropListEnents(m_pUISlotBagList);
    ...
    m_slots_array[BAG_SLOT] = m_pUISlotBagList;
    // pop-up menu
