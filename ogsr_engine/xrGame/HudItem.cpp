...
SAFEMODE : {
        ...
        Fvector safemode_offs = m_safe_mode_offset[0][0];
        Fvector safemode_rot = m_safe_mode_offset[0][1];
        float fSafeModeMaxTime = 0.4f;

        const float fSafeModePerUpd = Device.fTimeDelta / fSafeModeMaxTime;

        if (issafemode)
        {
            mfSafeMode_SetFactor += fSafeModePerUpd;
            clamp(mfSafeMode_SetFactor, 0.0f, 1.0f);
        }

        else
        {
            if (mfSafeMode_SetFactor < 0.0f)
            {
                mfSafeMode_SetFactor += fSafeModePerUpd;
                clamp(mfSafeMode_SetFactor, -1.0f, 0.0f);
            }
            else
            {
                mfSafeMode_SetFactor -= fSafeModePerUpd;
        ...
        safemode_offs.mul(mfSafeMode_SetFactor);
        safemode_rot.mul(-PI / 180.f); 
        safemode_rot.mul(mfSafeMode_SetFactor);
