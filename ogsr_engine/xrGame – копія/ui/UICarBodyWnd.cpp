...
void CUICarBodyWnd::SendMessage(CUIWindow* pWnd, s16 msg, void* pData)
{
    ...
    else if (pWnd == m_pUIPropertiesBox && msg == PROPERTY_CLICKED)
    {
        if (m_pUIPropertiesBox->GetClickedItem())
        {
            ...
            break;
            case INVENTORY_DETACH_BAYONET_ADDON: {
                auto wpn = smart_cast<CWeapon*>(CurrentIItem());
                wpn->Detach(wpn->GetBayonetName().c_str(), true);
                PlaySnd(eInvDetachAddon);
            }
            case INVENTORY_DETACH_RAIL_ADDON: {
                auto wpn = smart_cast<CWeapon*>(CurrentIItem());
                wpn->Detach(wpn->GetRailName().c_str(), true);
                PlaySnd(eInvDetachAddon);
            }
            break;
            case INVENTORY_DETACH_LASER_ADDON: {
                auto wpn = smart_cast<CWeapon*>(CurrentIItem());
                wpn->Detach(wpn->GetLaserName().c_str(), true);
                PlaySnd(eInvDetachAddon);
            }
            case INVENTORY_DETACH_TORCH_ADDON: {
                auto wpn = smart_cast<CWeapon*>(CurrentIItem());
                wpn->Detach(wpn->GetTorchName().c_str(), true);
                PlaySnd(eInvDetachAddon);
            }
            break;
            case INVENTORY_DETACH_TACTHANDLER_ADDON: {
                auto wpn = smart_cast<CWeapon*>(CurrentIItem());
                wpn->Detach(wpn->GetTactHandlerName().c_str(), true);
                PlaySnd(eInvDetachAddon);
            }
            break;
...
void CUICarBodyWnd::ActivatePropertiesBox()
{
    ...
    if (pWeapon)
    {
        ...
        {
            m_pUIPropertiesBox->AddItem("st_detach_bayonet", NULL, INVENTORY_DETACH_BAYONET_ADDON);
            b_show = true;
        }
        if (pWeapon->RailAttachable() && pWeapon->IsRailAttached())
        {
            m_pUIPropertiesBox->AddItem("st_detach_rail", NULL, INVENTORY_DETACH_RAIL_ADDON);
            b_show = true;
        }
        if (pWeapon->LaserAttachable() && pWeapon->IsLaserAttached())
        {
            m_pUIPropertiesBox->AddItem("st_detach_laser", NULL, INVENTORY_DETACH_LASER_ADDON);
            b_show = true;
        }
        if (pWeapon->TorchAttachable() && pWeapon->IsTorchAttached())
        {
            m_pUIPropertiesBox->AddItem("st_detach_torch", NULL, INVENTORY_DETACH_TORCH_ADDON);
            b_show = true;
        }
        if (pWeapon->TactHandlerAttachable() && pWeapon->IsTactHandlerAttached())
        {
            m_pUIPropertiesBox->AddItem("st_detach_tact_handler", NULL, INVENTORY_DETACH_TACTHANDLER_ADDON);
            b_show = true;
        }
