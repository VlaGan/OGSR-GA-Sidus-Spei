...
void CUIInventoryWnd::ActivatePropertiesBox()
{
    ...
    if (pWeapon)
    {
        ...
        if (pWeapon->BayonetAttachable() && pWeapon->IsBayonetAttached())
        {
            UIPropertiesBox.AddItem("st_detach_bayonet", NULL, INVENTORY_DETACH_BAYONET_ADDON);
            b_show = true;
        }
        if (pWeapon->RailAttachable() && pWeapon->IsRailAttached())
        {
            UIPropertiesBox.AddItem("st_detach_rail", NULL, INVENTORY_DETACH_RAIL_ADDON);
            b_show = true;
        }
        ...
        if (pWeapon->TorchAttachable() && pWeapon->IsTorchAttached())
        {
            UIPropertiesBox.AddItem("st_detach_torch", NULL, INVENTORY_DETACH_TORCH_ADDON);
            b_show = true;
        }
        if (pWeapon->TactHandlerAttachable() && pWeapon->IsTactHandlerAttached())
        {
            UIPropertiesBox.AddItem("st_detach_tact_handler", NULL, INVENTORY_DETACH_TACTHANDLER_ADDON);
            b_show = true;
        }
...
void CUIInventoryWnd::ProcessPropertiesBoxClicked()
{
    if (UIPropertiesBox.GetClickedItem())
    {
        ...
        case INVENTORY_DETACH_BAYONET_ADDON: DetachAddon(*(smart_cast<CWeapon*>(CurrentIItem()))->GetBayonetName()); break;
        case INVENTORY_DETACH_RAIL_ADDON: {
            CWeapon* pWPN = smart_cast<CWeapon*>(CurrentIItem());
            DetachAddon(*pWPN->GetRailName());

            // Если присоедены фонарь и лазер на планку - кутаем и их
            if (pWPN->IsLaserAttached())
                DetachAddon(*pWPN->GetLaserName());
            if (pWPN->IsTorchAttached())
                DetachAddon(*pWPN->GetTorchName());
        }
        break;
