...
CUIWeaponCellItem::CUIWeaponCellItem(CWeapon* itm) : inherited(itm)
{
    ...
    m_addons[eBayonet] = NULL;
    m_addons[eRail] = NULL;
    m_addons[eLaser] = NULL;
    m_addons[eTorch] = NULL;
    m_addons[eTactHandler] = NULL;
    ...
    if (itm->BayonetAttachable())
        m_addon_offset[eBayonet].set(object()->GetBayonetX(), object()->GetBayonetY());

    if (itm->RailAttachable())
        m_addon_offset[eRail].set(object()->GetRailX(), object()->GetRailY());

    if (itm->LaserAttachable())
        m_addon_offset[eLaser].set(object()->GetLaserX(), object()->GetLaserY());

    if (itm->TorchAttachable())
        m_addon_offset[eTorch].set(object()->GetTorchX(), object()->GetTorchY());

    if (itm->TactHandlerAttachable())
        m_addon_offset[eTactHandler].set(object()->GetTactHandlerX(), object()->GetTactHandlerY());
}
...
bool CUIWeaponCellItem::is_bayonet() { return object()->BayonetAttachable() && object()->IsBayonetAttached(); }

bool CUIWeaponCellItem::is_rail() { return object()->RailAttachable() && object()->IsRailAttached(); }

bool CUIWeaponCellItem::is_laser() { return object()->LaserAttachable() && object()->IsLaserAttached(); }

bool CUIWeaponCellItem::is_torch() { return object()->TorchAttachable() && object()->IsTorchAttached(); }

bool CUIWeaponCellItem::is_tacthandler() { return object()->TactHandlerAttachable() && object()->IsTactHandlerAttached(); }
...
void CUIWeaponCellItem::Update()
{
    ...
    if (object()->BayonetAttachable())
    {
        if (object()->IsBayonetAttached())
        {
            if (!GetIcon(eBayonet) || bForceReInitAddons)
            {
                CIconParams params(object()->GetBayonetName());
                CreateIcon(eBayonet, params);
                InitAddon(GetIcon(eBayonet), params, m_addon_offset[eBayonet], Heading());
            }
        }
        else
        {
            if (m_addons[eBayonet])
                DestroyIcon(eBayonet);
        }
    }

    if (object()->RailAttachable())
    {
        if (object()->IsRailAttached())
        {
            if (!GetIcon(eRail) || bForceReInitAddons)
            {
                CIconParams params(object()->GetRailName());
                CreateIcon(eRail, params);
                InitAddon(GetIcon(eRail), params, m_addon_offset[eRail], Heading());
            }
        }
        else
        {
    ...
    if (object()->TorchAttachable())
    {
        if (object()->IsTorchAttached())
        {
            if (!GetIcon(eTorch) || bForceReInitAddons)
            {
                CIconParams params(object()->GetTorchName());
                CreateIcon(eTorch, params);
                InitAddon(GetIcon(eTorch), params, m_addon_offset[eTorch], Heading());
            }
        }
        else
        {
    ...
    if (object()->TactHandlerAttachable())
    {
        if (object()->IsTactHandlerAttached())
        {
            if (!GetIcon(eTactHandler) || bForceReInitAddons)
            {
                CIconParams params(object()->GetTactHandlerName());
                CreateIcon(eTactHandler, params);
                InitAddon(GetIcon(eTactHandler), params, m_addon_offset[eTactHandler], Heading());
            }
        }
        else
        {
            if (m_addons[eTactHandler])
                DestroyIcon(eTactHandler);
        }
    }
...
void CUIWeaponCellItem::OnAfterChild(CUIDragDropListEx* parent_list)
{
    ...
    CUIStatic* s_bayonet = is_bayonet() ? GetIcon(eBayonet) : NULL;
    CUIStatic* s_rail = is_rail() ? GetIcon(eRail) : NULL;
    CUIStatic* s_laser = is_laser() ? GetIcon(eLaser) : NULL;
    CUIStatic* s_torch = is_torch() ? GetIcon(eTorch) : NULL;
    CUIStatic* s_tacthandler = is_tacthandler() ? GetIcon(eTactHandler) : NULL;
    InitAllAddons(s_silencer, s_scope, s_launcher, s_bayonet, s_rail, s_laser, s_torch, s_tacthandler, parent_list->GetVerticalPlacement());
}

void CUIWeaponCellItem::InitAllAddons(CUIStatic* s_silencer, CUIStatic* s_scope, CUIStatic* s_launcher, CUIStatic* s_bayonet, CUIStatic* s_rail, CUIStatic* s_laser, 
    CUIStatic* s_torch, CUIStatic* s_tacthandler, bool b_vertical)
{
    CIconParams params;
    ...
    if (s_bayonet)
    {
        params.Load(*object()->GetBayonetName());
        params.set_shader(s_bayonet);
        InitAddon(s_bayonet, params, m_addon_offset[eBayonet], b_vertical);
    }
    if (s_rail)
    {
        params.Load(*object()->GetRailName());
        params.set_shader(s_rail);
        InitAddon(s_rail, params, m_addon_offset[eRail], b_vertical);
    }
    if (s_laser)
    {
    ...
    if (s_torch)
    {
        params.Load(*object()->GetTorchName());
        params.set_shader(s_torch);
        InitAddon(s_torch, params, m_addon_offset[eTorch], b_vertical);
    }
    if (s_tacthandler)
    {
        params.Load(*object()->GetTactHandlerName());
        params.set_shader(s_tacthandler);
        InitAddon(s_tacthandler, params, m_addon_offset[eTactHandler], b_vertical);
    }
}
...
CUIDragItem* CUIWeaponCellItem::CreateDragItem()
{
    ...
    CUIStatic* s_bayonet = nullptr;
    CUIStatic* s_rail = nullptr;
    CUIStatic* s_laser = nullptr;
    CUIStatic* s_torch = nullptr;
    CUIStatic* s_tacthandler = nullptr;
    ...
    {
        params.Load(object()->GetBayonetName());
        s_bayonet = MakeAddonStatic(i, params);
    }
    if (GetIcon(eRail))
    {
        params.Load(object()->GetRailName());
        s_rail = MakeAddonStatic(i, params);
    }
    if (GetIcon(eLaser))
    {
        params.Load(object()->GetLaserName());
    ...
    {
        params.Load(object()->GetTorchName());
        s_torch = MakeAddonStatic(i, params);
    }
    if (GetIcon(eTactHandler))
    {
        params.Load(object()->GetTactHandlerName());
        s_tacthandler = MakeAddonStatic(i, params);
    }
    ...
    */
    ...
    InitAllAddons(s_silencer, s_scope, s_launcher, s_bayonet, s_rail, s_laser, s_torch, s_tacthandler, false);
    if (Heading())
